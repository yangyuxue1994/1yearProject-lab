...
def read_data(subjNum):
	...
	path=PATH+"ASP_"+VER+"_ori/ASP_"+VER+"_"+str(subjNum)+"_data.csv"
...
def read_randorv4(subjNum):
	col=['P_type', 'P_image_ID', 'P_sentence','T_verb','verif_ans']
	path = PATH+"ASP_"+VER+"_new/ASP_"+VER+"_"+str(subjNum)+"_randOrder.csv"
	print (path)
	df = pd.read_csv(path, header=0, index_col=False, usecols=col)
	return df
